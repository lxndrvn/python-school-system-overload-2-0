...
class QuestionInterface:
    ...
    def check_questions():
        ...
        print('There are', len(Question.select().where(condition)),
    ...
    def accept_new_questions(user=None):
        for question in Question.select().where(Question.status == 'NEW'):
            ...
            question.mentor = random.choice(
                [mentor for mentor in Mentor.select().where(Mentor.school == question.applicant.school)])
